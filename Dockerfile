...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends -o=Dpkg::Use-Pty=0 \
    caffe-cuda caffe-tools-cuda 
    
RUN apt-get install -y --no-install-recommends \
    dvipng
    
RUN apt-get install -y --no-install-recommends \
    freeglut3-dev gsl-bin minicom mrpt-apps
    
RUN apt-get install -y --no-install-recommends \
    libacl1 libatlas-base-dev libattr1 libaudit-common libaudit1 libavcodec-extra libblkid1 libboost-dev \
...
#RUN apt-get install -y --no-install-recommends \
#     lib-nite2-dev libnewton-dev libnmea-dev  libo3d3xx-camera libo3d3xx-framegrabber libo3d3xx-image libo3d3xx-pcicclient libogdf-dev
